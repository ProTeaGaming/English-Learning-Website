...
export default function FillGap() {
  ...
  const start = () => {
    ...
    const qs = shuffle(pool).slice(0, n).map(buildGapQuestion);
    setQuestions(qs);
    setIdx(0);
    setScore(0);
    setAnswer(null);
  ...
  if (stage === "setup") {
    return (
      <section>
        <PageHead title="Fill the Gap" desc="Choose the word that correctly completes each sentence." />
          ...
          <div className="count-row">
            {GAP_COUNTS.map((c) => (
              <button
                ...
                onClick={() => setCount(c)}
              >
                {c === "all" ? "All sentences" : `${c} sentences`}
              </button>
            ))}
          </div>
          <TopicCefrFilter
            ...
            resultLabel={`${pool.length} sentence${pool.length !== 1 ? "s" : ""} match`}
          />
          {pool.length === 0 ? (
            <p className="text-c2 text-[.85rem] mb-4">No sentences match these filters — try widening your selection.</p>
          ) : null}
          <button className="btn" onClick={start} disabled={!pool.length}>Start Exercise</button>
        </div>
      </section>
    );
  }
      ...
      <section>
        ...
        <div className="quiz-wrap">
          ...
          <div className="flex justify-between text-[.85rem] text-muted mb-3.5 font-semibold">
            <span>Sentence {idx + 1} of {questions.length}</span>
              ...
              <>
                ...
                <div className="flex justify-end mt-4">
                  <button className="btn" onClick={next}>
                    {idx + 1 === questions.length ? "See Results" : "Next Sentence"}
                  </button>
                </div>
              </>
            )}
  ...
  return (
    <section>
      ...
      <div className="result-card">
        <h2 className="text-[1.3rem] text-muted mb-2">Exercise Complete</h2>
