...
export default function Quiz({ learned }) {
  ...
  const start = () => {
    ...
    const qs = shuffle(pool)
      .slice(0, n)
      .map((word) => buildQuestion(word, mode === "mixed" ? randomMixedMode(word) : mode));
    setQuestions(qs);
    setIdx(0);
    setScore(0);
    setAnswer(null);
  ...
  if (stage === "setup") {
    return (
      <section>
        <PageHead title="Quiz" desc="Test your knowledge with multiple question types." />
        <div className="setup-card">
          <h2 className="text-[1.4rem] mb-2">Choose a quiz mode</h2>
          <p className="text-muted text-[.92rem] mb-6">Pick a question style, choose how many questions, then start.</p>
          ...
          <button className="btn" onClick={start} disabled={!pool.length}>Start Quiz</button>
        </div>
      </section>
    );
  }
      ...
      <section>
        ...
        <div className="quiz-wrap">
          ...
          <div className="q-card" ref={cardRef}>
            <div className="text-[.85rem] text-muted uppercase tracking-wider font-bold mb-2.5">{q.prompt}</div>
            <div className="text-[1.25rem] font-bold font-sora mb-6 leading-relaxed">{q.text}</div>
  ...
  return (
    <section>
      ...
      <div className="result-card">
        <h2 className="text-[1.3rem] text-muted mb-2">Quiz Complete</h2>
        ...
        <div className="flex gap-2.5 justify-center flex-wrap">
          ...
          <button className="btn secondary" onClick={() => setStage("setup")}>Change Mode</button>
        </div>
      </div>
    </section>
  );
