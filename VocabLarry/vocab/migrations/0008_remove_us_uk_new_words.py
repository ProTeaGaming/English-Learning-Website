...
# Mirrors 0007_add_us_uk_new_words.NEW_WORDS exactly (duplicated here rather
# than imported, since migrations must stay self-contained and independently
# replayable). This migration reverses 0007: the 18 new words were scope
# creep (the user only wanted existing US-form words to gain a UK-mode
# override, not brand-new vocabulary added to the dataset), so they're
# removed and the dataset returns to exactly 5,000 words.
NEW_WORDS = [
...
def restore_words(apps, schema_editor):
    Word = apps.get_model('vocab', 'Word')
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ('vocab', '0007_add_us_uk_new_words'),
    ]

    operations = [
        migrations.RunPython(remove_words, restore_words),
    ]
